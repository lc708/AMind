#!/usr/bin/env python3
"""Verify the self-contained Nuwa v1 release package using only the stdlib."""
...
DEFAULT_RELEASE = ROOT / "release/nuwa-v1"
...
def resolve_inside(root: Path, relative: str) -> Path:
    expected_prefix = "release/nuwa-v1/"
...
def verify(release_root: Path) -> dict[str, int]:
    ...
    require(manifest.get("schema_name") == "nuwa-v1-release-manifest", "invalid manifest schema")
    require(audit.get("schema_name") == "nuwa-v1-release-audit", "invalid release audit schema")
    ...
    require(len(artifacts) == manifest["counts"]["published_artifacts"] == 20, "artifact count drift")
    seen = set()
