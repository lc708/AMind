#!/usr/bin/env python3
"""Assemble the deterministic, GitHub-sized AMind v1 public release package."""
...
POLICY = ROOT / "config/amind-v1-release-policy.json"
...
TESTS = ROOT / "scripts/test_build_amind_v1_release.py"
CLI_TESTS = ROOT / "scripts/test_amind_v1.py"
...
def build() -> tuple[dict[Path, bytes], dict[str, Any]]:
    ...
    require(policy.get("schema_name") == "amind-v1-release-policy", "invalid release policy schema")
    ...
    require(len(artifact_rows) == 21, "release artifact count drift")

    manifest = {
        "schema_name": "amind-v1-release-manifest",
        "schema_version": 1,
        ...
        "compatibility": {
            "legacy_internal_identifier_prefixes": ["nuwa1-", "nuwa1804-", "nuwa-v1-"],
            "reason": "Stable cross-file identifiers are retained so evidence links remain reproducible; they are not the public release name.",
        },
    }
    ...
    input_paths = [POLICY, EVALUATION_AUDIT, CLAIM_AUDIT, SYNTHESIS_AUDIT, Path(__file__).resolve(), TESTS, CLI_TESTS]
    input_fingerprints = {
    ...
    audit = {
        "schema_name": "amind-v1-release-audit",
        "schema_version": 1,
        "release_id": policy["release_id"],
        "release_name": policy["release_name"],
        "verdict": "PASS",
        ...
        "invariants": {
            ...
            "public_release_named_amind_v1": True,
            "installation_required": False,
            "codex_skill_required": False,
            "legacy_internal_identifiers_documented": True,
